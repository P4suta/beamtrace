...
* **cli:** discover `beamtrace.toml` by walking from the working directory up to the repository boundary (`.git` directory or worktree file); `config check` prints the discovered path, the `init` template documents itself and ships no active placeholder profile, and `trigger` validation reports the MFA parser's message — the format now has a reference in `docs/project-config.md`
