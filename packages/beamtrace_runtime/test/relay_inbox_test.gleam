...
pub fn exact_inbox_truncates_without_replacing_accepted_frames_test() {
  ...
  relay_inbox.append(store, "relay-1", 1, relay_inbox.Exact, "one", 1000)
  |> should.equal(Ok(relay_inbox.Accepted))
  relay_inbox.append(store, "relay-1", 2, relay_inbox.Exact, "two", 1001)
  |> should.equal(Ok(relay_inbox.Accepted))
  relay_inbox.append(store, "relay-1", 3, relay_inbox.Exact, "three", 1002)
  |> should.equal(Ok(relay_inbox.Truncated("hub_inbox_budget")))

  relay_inbox.snapshot(store, "relay-1")
  |> should.equal([
    relay_inbox.Payload(1, "one", 1000),
    relay_inbox.Payload(2, "two", 1001),
  ])
...
pub fn live_inbox_drops_oldest_and_surfaces_an_explicit_gap_test() {
  let store = relay_inbox.new(max_frames: 2, max_bytes: 64)
  relay_inbox.append(store, "relay-live", 1, relay_inbox.Live, "one", 1000)
  |> should.equal(Ok(relay_inbox.Accepted))
  relay_inbox.append(store, "relay-live", 2, relay_inbox.Live, "two", 1001)
  |> should.equal(Ok(relay_inbox.Accepted))
  relay_inbox.append(store, "relay-live", 3, relay_inbox.Live, "three", 1002)
  |> should.equal(Ok(relay_inbox.Accepted))
  ...
  |> should.equal([
    ...
    relay_inbox.Payload(2, "two", 1001),
    relay_inbox.Payload(3, "three", 1002),
  ])
  relay_inbox.close(store)
}
...
pub fn inbox_rejects_invalid_identity_sequence_and_oversized_single_frame_test() {
  ...
  relay_inbox.append(store, "", 1, relay_inbox.Exact, "one", 1000)
  |> should.equal(Error("invalid_frame"))
  relay_inbox.append(store, "relay-1", 0, relay_inbox.Exact, "one", 1000)
  |> should.equal(Error("invalid_frame"))
  relay_inbox.append(store, "relay-1", 1, relay_inbox.Exact, "12345", 1000)
...
pub fn inbox_window_is_ordered_bounded_and_reports_the_total_test() {
  ...
  relay_inbox.append(store, "relay-page", 1, relay_inbox.Exact, "one", 1000)
  |> should.equal(Ok(relay_inbox.Accepted))
  relay_inbox.append(store, "relay-page", 2, relay_inbox.Exact, "two", 1001)
  |> should.equal(Ok(relay_inbox.Accepted))
  relay_inbox.append(store, "relay-page", 3, relay_inbox.Exact, "three", 1002)
  |> should.equal(Ok(relay_inbox.Accepted))

  relay_inbox.window(store, "relay-page", start: 1, limit: 1)
  |> should.equal(
    Ok(relay_inbox.Window(
      entries: [relay_inbox.Payload(2, "two", 1001)],
      total: 3,
      start: 1,
      limit: 1,
    )),
