...
$snapshot = Join-Path $packageRoot 'test/package-interface-v0.4.json'
...
try {
    ...
    if (-not $matches) {
        throw 'beamtrace_core public API changed. Review it, then intentionally regenerate test/package-interface-v0.4.json.'
    }
}
finally {
...
Write-Host 'beamtrace_core package interface matches the v0.4 snapshot.'
exit 0
